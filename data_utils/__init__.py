...
def make_dataset(path, seq_length, text_key, label_key, lazy=False, process_fn=None, split=[1.], delim=',', loose=False,
                 ...
                 non_binary_cols=None, enc_model_type='bert', max_dataset_size=None, num_facets=3, use_hard_neg=False,**kwargs):
    ...
    if should_split(split):
        ...
        if ds_type.lower() == 'bert':
            ...
            ds = [ds_subtype(d, max_seq_len=seq_length, presplit_sentences=presplit_sentences, max_dataset_size=max_dataset_size, num_facets=num_facets, use_hard_neg = use_hard_neg) for d in ds]
    else:
        if ds_type.lower() == 'bert':
            presplit_sentences = kwargs['presplit_sentences'] if 'presplit_sentences' in kwargs else False
            ds = ds_subtype(ds, max_seq_len=seq_length, presplit_sentences=presplit_sentences, max_dataset_size=max_dataset_size, num_facets=num_facets, use_hard_neg = use_hard_neg)
    return ds, tokenizer
