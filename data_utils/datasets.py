...
class bert_dataset(data.Dataset):
    ...
    def __init__(self, ds, max_seq_len=512, mask_lm_prob=.15, max_preds_per_seq=None, short_seq_prob=0.01,
                 presplit_sentences=False, max_dataset_size=None, num_facets=3, use_hard_neg=False, **kwargs):
        self.avg_len = []
        self.ds = ds
    ...
    def create_random_sentencepair(self, rng):
        ...
        # Contiguous multiple sequences
            ...
            if "rg" in self.modes or "fs" in self.modes: 
                ...
                target_seq_len = self.max_seq_len * self.num_seq_returned
                #if self.use_hard_neg:
    ...
    def create_masked_lm_predictions(self, tokens, token_labels, mask_lm_prob, max_preds_per_seq,
        ...
        if self.use_hard_neg == 2:
            token_types = [[self.tokenizer.get_type('str' + str( int(i/2) % len(tokens) ) ).Id] * len(tokens[i]) for i in range(len(tokens))]
        else:
            token_types = [[self.tokenizer.get_type('str' + str(i % self.num_sent_per_seq)).Id] * len(tokens[i]) for i in range(len(tokens))]
        ...
        elif self.num_seq_returned == 3 and self.num_sent_per_seq == 2:
            assert len(tokens) == 6
            half = 2
            t1, tt1, tl1 = self.concat_sentences(tokens[:half], token_types[:half], {k: v[:half] for k, v in token_labels.items()})
            t2, tt2, tl2 = self.concat_sentences(tokens[half:2*half], token_types[half:2*half], {k: v[half:2*half] for k, v in token_labels.items()})
            t3, tt3, tl3 = self.concat_sentences(tokens[2*half:], token_types[2*half:], {k: v[2*half:] for k, v in token_labels.items()})
            tokens, token_types, token_labels = t1 + t2 + t3, tt1 + tt2 + tt3, {k: tl1[k] + tl2[k] + tl3[k] for k in token_labels}
        ...
        for i in range(len(tokens)):
            ...
            for k in token_labels.keys():
                #token_labels[k][i] = [0.] + [0.]*self.num_facets + token_labels[k][i] + [0.]
