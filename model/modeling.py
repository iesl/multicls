...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, checkpoint_activations=False, facet_pos_emb = None, facet_lin_emb = None,
                diversify_hidden_layer = -1, facet_mode="", output_attentions=False):
        all_encoder_layers = []
        ...
        def custom(start, end):
            def custom_forward(*inputs):
                ...
                x_ = inputs[0]
                for layer in layers:
                    x_ = layer(x_, inputs[1], output_attentions)
                return x_
        ...
        if checkpoint_activations:
            ...
            for i,layer_module in enumerate(self.layer):
                if facet_pos_emb is not None:
                    layer_num, num_facet, emb_size = facet_pos_emb.size()
                    hidden_states[:,1:1+num_facet,:] += facet_pos_emb[i,:,:].unsqueeze(dim=0)
                    
                if current_idx < len(diversify_hidden_layer) and i == diversify_hidden_layer[current_idx]:
                    
                    #print(facet_pos_emb.size())
                    #print(hidden_states.size())
                    if facet_mode == "buggy":
                        num_facet = len(facet_lin_emb)
                    else:
                        num_facet = len(facet_lin_emb[0])
                    if facet_mode == "residual":
                        hidden_states = torch.cat([hidden_states[:,0,:].unsqueeze(dim=1)] + [(facet_lin_emb[current_idx][j-1](hidden_states[:,j,:]) + hidden_states[:,j,:]).unsqueeze(dim=1) for j in range(1,num_facet+1)] + [hidden_states[:,num_facet+1:,:]],dim=1)
                    else:
                        hidden_states = torch.cat([hidden_states[:,0,:].unsqueeze(dim=1)] + [facet_lin_emb[current_idx][j-1](hidden_states[:,j,:]).unsqueeze(dim=1) for j in range(1,num_facet+1)] + [hidden_states[:,num_facet+1:,:]],dim=1)
                    current_idx += 1
                    #for j in range(1,num_facet+1):
                    #    hidden_states[:,j,:] = facet_pos_emb[j-1](hidden_states[:,j,:])
                layer_outputs = layer_module(hidden_states, attention_mask, output_attentions)
                hidden_states = layer_outputs[0]
                #if facet_pos_emb is not None:
                #    assert diversify_hidden_layer > 0
        ...
        if not output_all_encoded_layers or checkpoint_activations:
            all_encoder_layers.append(hidden_states)
        outputs = (all_encoder_layers, all_self_attentions) if output_attentions else (all_encoder_layers,)
        return outputs
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, task_ids=None, attention_mask=None, output_all_encoded_layers=True,
                checkpoint_activations=False, facet_pos_emb = None, facet_lin_emb = None, diversify_hidden_layer = -1,
                facet_mode = "", output_attentions=None, num_facets=5, facet_mask=0):
        ...
        if facet_mask != 0:
            ...
            attention_facet_mask = torch.ones(1+num_facets, 1+num_facets).cuda()
            attention_facet_mask = attention_facet_mask - torch.diag(attention_facet_mask[0])
            attention_facet_mask[0, :] = attention_facet_mask[0, :] * 0
            attention_facet_mask[:, 0] = attention_facet_mask[:, 0] * 0
            attention_facet_mask = attention_facet_mask * facet_mask
            attention_facet_mask = attention_facet_mask.to(dtype=next(self.encoder.parameters()).dtype)
            #(batch, 1, from_seq_length, to_seq_length)
            extended_attention_mask = extended_attention_mask.expand(extended_attention_mask.shape[0],
                                                                     1, seq_length, seq_length).clone()
            extended_attention_mask[:, :, :num_facets + 1, :num_facets + 1] += attention_facet_mask.expand_as(
                extended_attention_mask[:, :, :num_facets + 1, :num_facets + 1])
        ...
        encoded_output = self.encoder(embedding_output,
                                      ...
                                      checkpoint_activations=checkpoint_activations, facet_pos_emb = facet_pos_emb,
                                      facet_lin_emb = facet_lin_emb, diversify_hidden_layer = diversify_hidden_layer,
                                      facet_mode=facet_mode, output_attentions=output_attentions)
        encoded_layers = encoded_output[0]
        ...
        outputs = (encoded_layers, pooled_output, encoded_output[1]) if output_attentions else (encoded_layers, pooled_output)
