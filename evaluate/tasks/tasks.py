...
def select_list(data, sample_idx):
    return [data[idx] for idx in sample_idx]
...
def process_single_pair_task_split(split, indexers, num_facet, is_pair=True, classification=True):
    """
    ...
    def _make_instance(input1, input2, labels, idx):
        ...
        d["sent1_str"] = MetadataField(" ".join(input1[1+num_facet:-1]))
        if is_using_bert and is_pair:
            ...
            d["sent2_str"] = MetadataField(" ".join(input2[1+num_facet:-1]))
        else:
...
class SingleClassificationTask(ClassificationTask):
    ...
    def process_split(self, split, indexers, num_facet) -> Iterable[Type[Instance]]:
        """ Process split text into a list of AllenNLP Instances. """
        return process_single_pair_task_split(split, indexers, num_facet, is_pair=False)
...
class PairClassificationTask(ClassificationTask):
    ...
    def process_split(self, split, indexers, num_facet) -> Iterable[Type[Instance]]:
        """ Process split text into a list of AllenNLP Instances. """
        return process_single_pair_task_split(split, indexers, num_facet, is_pair=True)
...
class PairRegressionTask(RegressionTask):
    ...
    def process_split(self, split, indexers, num_facet) -> Iterable[Type[Instance]]:
        """ Process split text into a list of AllenNLP Instances. """
        return process_single_pair_task_split(split, indexers, num_facet, is_pair=True, classification=False)
...
class CoLAAnalysisTask(SingleClassificationTask):
    ...
    def process_split(self, split, indexers, num_facet):
...
class BroadCoverageDiagnosticTask(GLUEDiagnosticTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        sent1s = [
            process_sentence(self._tokenizer_name, d["sentence1"], self.max_seq_len, insert_facet=True, num_facet=num_facet) for d in data
        ]
        sent2s = [
            process_sentence(self._tokenizer_name, d["sentence2"], self.max_seq_len, insert_facet=False) for d in data
        ]
...
class WinogenderTask(GLUEDiagnosticTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        self.train_data_text = load_pair_nli_jsonl(
            os.path.join(self.path, "AX-g.jsonl"), self._tokenizer_name, self.max_seq_len, targ_map, num_facet
        )
...
class RTESuperGLUETask(RTETask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        def _load_jsonl(data_file, num_facet, few_shot = [-1, 0, None]):
            data = [json.loads(d) for d in open(data_file, encoding="utf-8")]
            sent1s, sent2s, trgs, idxs = [], [], [], []
            for example in data:
                sent1s.append(
                    process_sentence(self._tokenizer_name, example["premise"], self.max_seq_len, insert_facet=True, num_facet=num_facet)
                )
                sent2s.append(
                    process_sentence(self._tokenizer_name, example["hypothesis"], self.max_seq_len, insert_facet=False)
                )
            ...
            if few_shot[0] == -1:
                ...
                sample_idx, seed = sample_training_data(len(sent1s), few_shot)
                #return [sent1s[sample_idx], sent2s[sample_idx], trgs[sample_idx], idxs[sample_idx]]
                return [ select_list(sent1s, sample_idx), select_list(sent2s, sample_idx), select_list(trgs, sample_idx), select_list(idxs, sample_idx)]

        self.train_data_text = _load_jsonl(os.path.join(self.path, "train.jsonl"), num_facet, few_shot)
        self.val_data_text = _load_jsonl(os.path.join(self.path, "val.jsonl"), num_facet)
        self.test_data_text = _load_jsonl(os.path.join(self.path, "test.jsonl"), num_facet)
...
class DisSentTask(PairClassificationTask):
    ...
    def load_data_for_path(self, path, few_shot, num_facet):
        """ Load data """
        assert few_shot[0] == -1
        with open(path, "r") as txt_fh:
            for row in txt_fh:
                ...
                if len(row) != 3 or not (row[0] and row[1] and row[2]):
                    continue
                sent1 = process_sentence(self._tokenizer_name, row[0], self.max_seq_len, insert_facet=True, num_facet=num_facet)
                sent2 = process_sentence(self._tokenizer_name, row[1], self.max_seq_len, insert_facet=False)
                targ = int(row[2])
...
class SpanClassificationTask(Task):
    ...
    def _pad_tokens(self, tokens):
        ...
        if self.tokenizer_name.startswith("bert-"):
            ...
            return ["[CLS]"] + facet_input_list + tokens + ["[SEP]"]
        else:
            return [utils.SOS_TOK] + facet_input_list + tokens + [utils.EOS_TOK]
    ...
    def make_instance(self, record, idx, indexers) -> Type[Instance]:
        ...
        offset = 1 + self.num_facet
        for i in range(self.num_spans):
            example["span" + str(i + 1) + "s"] = ListField(
                [self._make_span_field(record["target"]["span" + str(i + 1)], text_field, offset=offset)]
            )
    ...
    def process_split(self, records, indexers, num_facet) -> Iterable[Type[Instance]]:
        """ Process split text into a list of AllenNLP Instances. """
...
class CommitmentTask(PairClassificationTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        def _load_data(data_file, num_facet, few_shot = [-1,0,None]):
            ...
            if few_shot[0] == -1:
                ...
                sample_idx, seed = sample_training_data(len(sent1s), few_shot)
                #permuted_idx = np.random.permutation(len(sent1s))
                #sample_idx = permuted_idx[:few_shot]
                #return [sent1s[sample_idx], sent2s[sample_idx], targs[sample_idx], idxs[sample_idx]]
                return [select_list(sent1s,sample_idx), select_list(sent2s, sample_idx), select_list(targs, sample_idx), select_list(idxs,sample_idx) ]
            #return [sent1s, sent2s, targs, idxs]

        self.train_data_text = _load_data(os.path.join(self.path, "train.jsonl"), num_facet, few_shot)
        self.val_data_text = _load_data(os.path.join(self.path, "val.jsonl"), num_facet)
        self.test_data_text = _load_data(os.path.join(self.path, "test.jsonl"), num_facet)
        self.sentences = (
            self.train_data_text[0]
            + self.val_data_text[0]
            + self.train_data_text[1]
...
class WiCTask(PairClassificationTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        def _process_preserving_word(sent, word, num_facet):
            ...
            sent_tok1 = process_sentence(self._tokenizer_name, sent_parts[0], self.max_seq_len, insert_facet=True, num_facet=num_facet)
            sent_tok2 = process_sentence(self._tokenizer_name, sent_parts[1], self.max_seq_len, insert_facet=False)
            sent_mid = process_sentence(self._tokenizer_name, word, self.max_seq_len, insert_facet=False)
        ...
        def _load_split(data_file, num_facet, few_shot=[-1,0,None]):
            ...
            with open(data_file, "r") as data_fh:
                for row in data_fh:
                    ...
                    sent1, start1, end1 = _process_preserving_word(sent1, word1, num_facet)
                    sent2, start2, end2 = _process_preserving_word(sent2, word2, num_facet)
                    sents1.append(sent1)
                    sents2.append(sent2)
                ...
                if few_shot[0] == -1:
                    ...
                    sample_idx, seed = sample_training_data(len(sents1), few_shot)
                    #permuted_idx = np.random.permutation(len(sents1))
                    #sample_idx = permuted_idx[:few_shot]
                    return [select_list(sents1, sample_idx), select_list(sents2, sample_idx), select_list(idxs1, sample_idx), select_list(idxs2, sample_idx), select_list(trgs, sample_idx), select_list(idxs, sample_idx)]

        self.train_data_text = _load_split(os.path.join(self.path, "train.jsonl"), num_facet, few_shot)
        self.val_data_text = _load_split(os.path.join(self.path, "val.jsonl"), num_facet)
        self.test_data_text = _load_split(os.path.join(self.path, "test.jsonl"), num_facet)
        self.sentences = (
            self.train_data_text[0]
            + self.train_data_text[1]
            + self.val_data_text[0]
...
class COPATask(MultipleChoiceTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        def _load_split(data_file, num_facet, few_shot = [-1,0,None]):
            ...
            for example in data:
                ...
                choices = [
                    process_sentence(self._tokenizer_name, choice, self.max_seq_len, insert_facet=False)
                    for choice in [choice1, choice2]
                ]
                targ = example["label"] if "label" in example else 0
                contexts.append(process_sentence(self._tokenizer_name, context, self.max_seq_len, insert_facet=True, num_facet=num_facet))
                choicess.append(choices)
                questions.append(process_sentence(self._tokenizer_name, question, self.max_seq_len, insert_facet=False))
                targs.append(targ)
            #return [contexts, choicess, questions, targs]
            if few_shot[0] == -1:
                return [contexts, choicess, questions, targs]
            else:
                sample_idx, seed = sample_training_data(len(contexts), few_shot)
                #permuted_idx = np.random.permutation(len(contexts))
                #sample_idx = permuted_idx[:few_shot]
                #return [contexts[sample_idx], choicess[sample_idx], questions[sample_idx], targs[sample_idx]]
                return [select_list(contexts, sample_idx), select_list(choicess, sample_idx), select_list(questions, sample_idx), select_list(targs, sample_idx)]
    ...
    def process_split(self, split, indexers, num_facet) -> Iterable[Type[Instance]]:
        ...
        def _make_instance(context, choices, question, label, idx):
            d = {}
            d["question_str"] = MetadataField(" ".join(context[1+num_facet:-1]))
            ...
            for choice_idx, choice in enumerate(choices):
                ...
                d["choice%d_str" % choice_idx] = MetadataField(" ".join(choice[1+num_facet:-1]))
...
class SWAGTask(MultipleChoiceTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        def _load_split(data_file):
            ...
            for ex_idx, ex in data.iterrows():
                sent1 = process_sentence(self._tokenizer_name, ex["sent1"], self.max_seq_len, insert_facet=True, num_facet=num_facet)
                questions.append(sent1)
                sent2_prefix = ex["sent2"]
                choices = []
                for i in range(4):
                    choice = sent2_prefix + " " + ex["ending%d" % i]
                    choice = process_sentence(self._tokenizer_name, choice, self.max_seq_len, insert_facet=False)
        ...
        self.train_data_text = _load_split(os.path.join(self.path, "train.csv"), num_facet)
        self.val_data_text = _load_split(os.path.join(self.path, "val.csv"), num_facet)
        self.test_data_text = _load_split(os.path.join(self.path, "test.csv"), num_facet)
        self.sentences = (
            self.train_data_text[0]
            + self.val_data_text[0]
            + [choice for choices in self.train_data_text[1] for choice in choices]
    ...
    def process_split(self, split, indexers, num_facet) -> Iterable[Type[Instance]]:
        ...
        def _make_instance(question, choices, label, idx):
            d = {}
            d["question_str"] = MetadataField(" ".join(question[1+num_facet:-1]))
            if not is_using_bert:
...
class WinogradCoreferenceTask(SpanClassificationTask):
    ...
    def load_data(self, few_shot, num_facet):
        #assert few_shot[0] <0 or few_shot[0] > 600
        ...
        for split, filename in self._files_by_split.items():
            ...
            elif filename.endswith("val.jsonl"):
                iters_by_split[split] = load_span_data(
                    self.tokenizer_name, filename, label_fn=lambda x: int(x)
                )
            elif filename.endswith("train.jsonl"):
                iters_by_split[split] = load_span_data(
                    self.tokenizer_name, filename, few_shot_sample=few_shot, label_fn=lambda x: int(x)
                )
...
class BooleanQuestionTask(PairClassificationTask):
    ...
    def load_data(self, few_shot, num_facet):
        ...
        def _load_jsonl(data_file, num_facet, few_shot = [-1,0,None]):
            ...
            data = []
            for d in raw_data:
                #question = process_sentence(self._tokenizer_name, d["question"], self.max_seq_len, insert_facet=True, num_facet=num_facet)
                question = process_sentence(self._tokenizer_name, d["question"], self.max_seq_len, insert_facet=False)
                passage = process_sentence(self._tokenizer_name, d["passage"], self.max_seq_len, insert_facet=True, num_facet=num_facet)
                #passage = process_sentence(self._tokenizer_name, d["passage"], self.max_seq_len, insert_facet=False)
                #print(passage)
            ...
            if few_shot[0] == -1:
                ...
                sample_idx, seed = sample_training_data(len(data), few_shot)
                #permuted_idx = np.random.permutation(len(data))
                #sample_idx = permuted_idx[:few_shot]
                return select_list(data, sample_idx)
            #return data
    ...
    def process_split(self, split, indexers, num_facet) -> Iterable[Type[Instance]]:
        ...
        def _make_instance(d, idx):
            ...
            new_d["passage_str"] = MetadataField(" ".join(d["passage"][1+num_facet:-1]))
            if not is_using_bert:
