...
def evaluate(
    ...
    analyze_grad = model.sent_encoder._text_field_embedder.analyze_grad
    #if analyze_grad:
    #    model.train()
    #else:
    #    model.eval()
    ...
    for task in tasks:
        ...
        model_task = getattr(model,"%s_mdl" % task.name)
        if hasattr(model_task, "pooler"):
            model_task.pooler.init_facet_norm(record_grad = analyze_grad) ##To check
        ...
        for batch_idx, batch in enumerate(generator):
            ...
            cols = {"preds": preds}
            if hasattr(model_task, "pooler") and model_task.pooler.facet_emb_arr is not None:
                cols['facet_emb'] = model_task.pooler.facet_emb_arr
                if analyze_grad:
                    cols['facet_importance'] = model_task.pooler.importance_arr
        ...
        #getattr(model,"%s_mdl" % task.name).pooler.print_facet_norm_avg() ##To check
        
        if hasattr(model_task, "pooler"):
            model_task.pooler.print_facet_norm_avg()
