...
def get_run_name(args):
    ...
    run_name = run_name[:-2] + args.run_name_suffix + args.checkpoint + run_name[-2:]
    return run_name
...
def evaluate_and_write(args, model, tasks, splits_to_write):
    ...
    #results_tsv = os.path.join(args.exp_dir, "results_unnorm.tsv")
...
def build_trainer(
    ...
    #        "warmup_steps": int(params["max_epochs"] * 0.1 * n_tr_batches)
    #    }
    #)
...
class SamplingMultiTaskTrainer:
    def __init__(
        ...
        self.use_huggingface_warmup = True
    ...
    def _setup_training(
        self, tasks, batch_size, train_params, optimizer_params, scheduler_params, phase, args
    ):
        ...
        for task in tasks:
            ...
            if self._max_epochs > 0 and "t_total" in optimizer_params:
                ...
                # there are, set it.
                opt_params["t_total"] = min(task_info["n_tr_batches"] * self._max_epochs, args.target_train_max_vals * args.target_train_val_interval )
    ...
    def train(
        ...
        task_infos, metric_infos = self._setup_training(
            tasks, batch_size, train_params, optimizer_params, scheduler_params, phase, args
        )
        
        #assert len(tasks) == 1
        #n_tr_batches = task_infos[tasks[0].name]["n_tr_batches"]
        #validation_interval = min(validation_interval, 2 * n_tr_batches)
        ...
        if "t_total" in optimizer_params and self._max_epochs > 0:
            ...
            optimizer_params["t_total"] = min( n_epoch_steps * self._max_epochs, args.target_train_max_vals * args.target_train_val_interval )
        optimizer = Optimizer.from_params(train_params, optimizer_params)
        if self.use_huggingface_warmup:
            n_epoch_steps = sum([info["n_tr_batches"] for info in task_infos.values()])
            t_total = min( n_epoch_steps * self._max_epochs, args.target_train_max_vals * args.target_train_val_interval )
            warmup_ratio = self.warmup_ratio
            scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=t_total*warmup_ratio, num_training_steps=t_total)
        else:
        ...
        assert task_n_train_batches.shape[0] == 1
        n_tr_batches = task_n_train_batches[0]
        validation_interval = min(validation_interval, 2 * n_tr_batches)
        ...
        while not should_stop:
            ...
            for batch in itertools.islice(tr_generator, 1):
                ...
                if args.noise_ratio == 0:
                    ...
                    batch_noise = copy.deepcopy(batch)
                    labels = batch_noise['labels']
                    assert labels.dim() == 1
                    bsz = labels.size(0)
                    shuffle_number = max(int(args.noise_ratio * bsz), 2)
                    all_idx = list(range(bsz))
                    random.shuffle(all_idx)
                    shuffle_idx = all_idx[:shuffle_number]
                    labels_to_be_shuffled = labels[shuffle_idx]
                    batch_noise['labels'][shuffle_idx] = labels_to_be_shuffled[torch.randperm(shuffle_number)]
                    #log.info(batch_noise['labels'])
                    #log.info(batch['labels'])
                    output_dict = self._forward(batch_noise, task=task)
                ...
                if (n_step % args.accumulate_grad_iter)==0:
                    optimizer.step()
                    optimizer.zero_grad()
                    # step scheduler if it's not ReduceLROnPlateau
                    if self.use_huggingface_warmup:
                        scheduler.step()
                    elif hasattr(scheduler, "lr_scheduler") and not isinstance(scheduler.lr_scheduler, ReduceLROnPlateau):
            ...
            if n_step % validation_interval == 0:
                ...
                if should_save and new_best:
                    self._model_best = copy.deepcopy(self._model)
        ...
        for task in tasks:
            ...
            evaluate_and_write(args, self._model_best, [task], splits_to_write)
    ...
    def _calculate_validation_performance(
        ...
        model_task = getattr(self._model,"%s_mdl" % task.name)
        if hasattr(model_task, "pooler"):
            model_task.pooler.init_facet_norm() ##To check
        ...
        #getattr(self._model,"%s_mdl" % task.name).pooler.print_facet_norm_avg() ##To check
        if hasattr(model_task, "pooler"):
            model_task.pooler.print_facet_norm_avg() ##To check
    ...
    def _validate(self, val_pass, tasks, batch_size, periodic_save=True):
        ...
        for task in tasks + ["micro", "macro"]:
            ...
            if task_name == "macro" and hasattr(scheduler, "lr_scheduler") and isinstance(scheduler.lr_scheduler, ReduceLROnPlateau):
                ...
                log.info(
                    "\tBest result seen so far %s", metric_infos[metric]["best"][1]
                )
    ...
    def from_params(cls, model, serialization_dir, params):
        ...
        warmup_ratio = params.pop("warmup_ratio", 0.1)
