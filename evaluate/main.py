...
from .preprocess import build_tasks, parse_task_list_arg
from . import tasks as task_modules
from .trainer import build_trainer, evaluate_and_write, get_run_name
...
def add_required_args(args, cl_args):
    ...
    #if 'superglue' not in cl_args.config_file:
    if "data_dir" not in args:
        args["data_dir"] = glue_data_path
    args["project_dir"] = finetuned_path
    if "bert_config_file" not in args:
        args["bert_config_file"] = bert_config_file
    #args["bert_config_file"] = getattr(cl_args,"bert_config_file", bert_config_file)
    ...
    if args.load_target_train_checkpoint in ["none", "None", None]:
        ...
        if len(cl_args.checkpoint)>0:
            load_path = os.path.join(pretrained_path, args.exp_name.replace('_noise','').replace('_few_32','').replace('_few','').replace('_100_ry','').replace('_super_glue','').replace('_too_large',''), "ck", "model_{}_converted.pt".format(cl_args.checkpoint))
        else:
            load_path = os.path.join(pretrained_path, args.exp_name.replace('_noise','').replace('_few_32','').replace('_few','').replace('_100_ry','').replace('_super_glue','').replace('_too_large',''), "best/model_converted.pt")
        args["load_target_train_checkpoint"] = load_path

def get_num_facet(args):
    ...
    args.num_facet = max(facet_list)

def detect_existing_results(results_tsv, run_name, target_task_names):
    #exist_code_early = False
    #check_task = target_task_names[0]
    to_run_set = set(target_task_names)
    with open(results_tsv) as f_in:
        for line in f_in:
            fields = line.split('\t')
            if len(fields) != 2:
                continue
            method_name, score_name = fields
            to_run_list = list(to_run_set)
            for check_task in to_run_list:
                if run_name in method_name and check_task in score_name:
                    to_run_set.remove(check_task)
                    #exist_code_early = True
                    #break
    #return exist_code_early
    return to_run_set
...
def main(cl_arguments):
    ...
    #cl_args.overrides = cl_args.overrides.replace('+','|')
    args = config.params_from_file(cl_args.config_file, cl_args.overrides)
    #args.run_name = args.run_name.replace('|','+')
    
    add_required_args(args, cl_args)
    global pretrained_path
    args_train = config.params_from_file( os.path.join(pretrained_path, "training.conf") )
    print(args_train)
    # Check for deprecated arg names
    check_arg_name(args)
    args, seed = initial_setup(args, cl_args)
    args.num_facet = args_train.num_facet
    args.diversify_hidden_layer = args_train.diversify_hidden_layer
    args.diversify_mode = args_train.diversify_mode
    if hasattr(args_train,'unnorm_facet'):
        args.unnorm_facet = args_train.unnorm_facet
    args.facet_mode = ""
    if hasattr(args_train,'facet_mode'):
        args.facet_mode = args_train.facet_mode
    args.checkpoint = cl_args.checkpoint

    results_tsv = os.path.join(args.exp_dir, "results.tsv")
    run_name = get_run_name(args)
    target_task_names = parse_task_list_arg(args.target_tasks)
    to_run_set = detect_existing_results(results_tsv, run_name, target_task_names)
    to_run_set_remain = to_run_set 
    if len(to_run_set_remain) == 0:
        log.info("found the same experiment in results.tsv using " + str(target_task_names))
        sys.exit(0)
    ...
    pretrain_tasks, target_tasks, vocab, word_embs = build_tasks(args, to_run_set_remain)
    ...
    if args.do_target_task_training:
        ...
        for task in target_tasks_to_train:
            ...
            trainer, _, opt_params, schd_params = build_trainer(
                ...
                phase="target_train",
                n_tr_batches = math.ceil(task.n_train_examples / args.batch_size)
            )
