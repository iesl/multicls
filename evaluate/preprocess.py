...
from .tasks import (
    ...
    SMALL_GLUE_TASKS,
    LARGE_GLUE_TASKS,
    LARGE_MISS_GLUE_TASKS,
    TOO_LARGE_GLUE_TASKS,
    ALL_SUPERGLUE_TASKS,
    RECORD_TASK,
    FOCUS_TASK,
    FLEX_TASKS,
    SMALL_SUPERGLUE_TASKS,
    LARGE_SUPERGLUE_TASKS,
    LARGE_MISS_SUPERGLUE_TASKS,
    TOO_LARGE_SUPERGLUE_TASKS,
...
def _get_serialized_record_path(task_name, split, preproc_dir, few_shot, seed):
    """Get the canonical path for a serialized task split."""
    if split != 'train' or few_shot == -1:
        serialized_record_path = os.path.join(preproc_dir, "{:s}__{:s}_data".format(task_name, split))
    else:
        serialized_record_path = os.path.join(preproc_dir, "{:s}__{:s}_data_few_{}_seed_{}".format(task_name, split, few_shot, seed))
        
    return serialized_record_path


def _get_instance_generator(task_name, split, preproc_dir, few_shot=None, seed=None, fraction=None):
    ...
    """
    filename = _get_serialized_record_path(task_name, split, preproc_dir, few_shot, seed)
...
def _index_split(task, split, indexers, vocab, record_file, num_facet):
    ...
    #log.info(split)
    #log.info(task.few_shot)
    #log.info(len(split_text))
    instance_iter = task.process_split(split_text, indexers, num_facet)
...
def build_tasks(args, to_run_set_remain):
    ...
    # 1) create / load tasks
    tasks, pretrain_task_names, target_task_names = get_tasks(args, to_run_set_remain)
    for task in tasks:
    ...
    preproc_dir = os.path.join(args.exp_dir, "preproc_f"+str(args.num_facet)+ '_' + args.tokenizer)
    ...
    for task in tasks:
        ...
        for split in ALL_SPLITS:
            ...
            relative_path = _get_serialized_record_path(task.name, split, "preproc_f"+str(args.num_facet) + '_' + args.tokenizer , few_shot = args.few_shot, seed = args.random_seed)
            ...
            if force_reindex or not cache_found:
                ...
                record_file = _get_serialized_record_path(task.name, split, preproc_dir, few_shot = args.few_shot, seed = args.random_seed )
                if os.path.exists(record_file) and os.path.islink(record_file):
                    os.remove(record_file)

                _index_split(task, split, indexers, vocab, record_file, args.num_facet)
    ...
    for task in tasks:
        ...
        if task.name in pretrain_task_names:
            ...
            task.train_data = _get_instance_generator(
                task.name, "train", preproc_dir, few_shot = args.few_shot, seed = args.random_seed, fraction=args.pretrain_data_fraction
            )
        ...
        if task.name in target_task_names:
            ...
            task.train_data = _get_instance_generator(
                task.name, "train", preproc_dir, few_shot = args.few_shot, seed = args.random_seed, fraction=args.target_train_data_fraction
            )
...
def parse_task_list_arg(task_list):
    ...
    for task_name in task_list.split(","):
        ...
        elif task_name == "glue_small":
            task_names.extend(SMALL_GLUE_TASKS)
        elif task_name == "glue_large":
            task_names.extend(LARGE_GLUE_TASKS)
        elif task_name == "glue_large_miss":
            task_names.extend(LARGE_MISS_GLUE_TASKS)
        elif task_name == "glue_too_large":
            task_names.extend(TOO_LARGE_GLUE_TASKS)
        elif task_name == "glue_qqp":
            task_names.extend(QQP_TASK)
        elif task_name == "superglue":
            task_names.extend(ALL_SUPERGLUE_TASKS)
        elif task_name == "superglue_small":
            task_names.extend(SMALL_SUPERGLUE_TASKS)
        elif task_name == "superglue_large":
            task_names.extend(LARGE_SUPERGLUE_TASKS)
        elif task_name == "superglue_large_miss":
            task_names.extend(LARGE_MISS_SUPERGLUE_TASKS)
        elif task_name == "superglue_too_large":
            task_names.extend(TOO_LARGE_SUPERGLUE_TASKS)
        elif task_name == "superglue_record":
            task_names.extend(RECORD_TASK)
        elif task_name == "focus":
            task_names.extend(FOCUS_TASK)
        elif task_name == "flex":
            task_names.extend(FLEX_TASKS)
...
def _get_task(name, args, data_path, scratch_path, random_num_arr):
    ...
    pkl_path = os.path.join(scratch_path, "tasks_f"+str(args.num_facet), f"{name:s}.{args.tokenizer:s}.few_{args.few_shot}.seed_{args.random_seed}.pkl") #Todo: to save some disk space, we should only store the training data differently.
    ...
    if os.path.isfile(pkl_path) and not args.reload_tasks:
        ...
        task.load_data(few_shot = [args.few_shot, args.random_seed, random_num_arr], num_facet=args.num_facet)
...
def get_tasks(args, to_run_set_remain):
    ...
    target_task_names = list(to_run_set_remain)
    ...
    tasks = []
    random_num_arr = None
    if args.few_shot > 0 and len(args.random_file_path) > 0:
        with open(args.random_file_path,'rb') as f_in:
            random_num_arr = pickle.load(f_in)
    for name in task_names:
        task = _get_task(name, args, data_path=data_path, scratch_path=scratch_path, random_num_arr=random_num_arr)
        tasks.append(task)
