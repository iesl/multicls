...
def build_task_specific_modules(task, model, d_sent, d_emb, vocab, embedder, args):
    ...
    if isinstance(task, SingleClassificationTask):
        module = build_single_sentence_module(
            task=task, d_inp=d_sent, use_bert=model.use_bert, params=task_params, load_target_train_checkpoint=args.load_target_train_checkpoint, unnorm_facet = args.unnorm_facet, num_facet = args.num_facet, analyze_grad = args.analyze_grad, embedder_for_grad=model.sent_encoder._text_field_embedder, pooler_dropout = args.pooler_dropout, pooler_scalar_dropout = args.pooler_scalar_dropout)
        setattr(model, "%s_mdl" % task.name, module)
    elif isinstance(task, (PairClassificationTask, PairRegressionTask, PairOrdinalRegressionTask)):
        module = build_pair_sentence_module(task, d_sent, model=model, params=task_params, load_target_train_checkpoint=args.load_target_train_checkpoint, unnorm_facet = args.unnorm_facet, num_facet = args.num_facet, analyze_grad = args.analyze_grad, pooler_dropout = args.pooler_dropout, pooler_scalar_dropout = args.pooler_scalar_dropout )
        setattr(model, "%s_mdl" % task.name, module)
        ...
        module = build_multiple_choice_module(
            task, d_sent, use_bert=model.use_bert, params=task_params, load_target_train_checkpoint=args.load_target_train_checkpoint, unnorm_facet = args.unnorm_facet, num_facet = args.num_facet, analyze_grad = args.analyze_grad, embedder_for_grad=model.sent_encoder._text_field_embedder, pooler_dropout = args.pooler_dropout, pooler_scalar_dropout = args.pooler_scalar_dropout
        )
        setattr(model, "%s_mdl" % task.name, module)
        ...
        module = build_qa_module(task, d_sent, model.use_bert, task_params, load_target_train_checkpoint=args.load_target_train_checkpoint, unnorm_facet = args.unnorm_facet, num_facet = args.num_facet, analyze_grad = args.analyze_grad, embedder_for_grad=model.sent_encoder._text_field_embedder, pooler_dropout = args.pooler_dropout, pooler_scalar_dropout = args.pooler_scalar_dropout)
        setattr(model, "%s_mdl" % task.name, module)
    else:
...
def build_image_sent_module(task, d_inp, params):
    pooler = Pooler(project=True, d_inp=d_inp, d_proj=params["d_proj"], pool_type=params["pool_type"])
    return pooler

def build_single_sentence_module(task, d_inp: int, use_bert: bool, params: Params, load_target_train_checkpoint: str, unnorm_facet: bool, num_facet: int, analyze_grad: bool, embedder_for_grad, pooler_dropout, pooler_scalar_dropout):
    ...
    """
    pooler = Pooler(num_facet=num_facet,
        project=not use_bert, d_inp=d_inp, d_proj=params["d_proj"], pool_type=params["pool_type"], previous_state_path=load_target_train_checkpoint, unnorm_facet=unnorm_facet, analyze_grad=analyze_grad, embedder_for_grad = embedder_for_grad, pooler_dropout = pooler_dropout, pooler_scalar_dropout = pooler_scalar_dropout)
...
def build_pair_sentence_module(task, d_inp, model, params, load_target_train_checkpoint,unnorm_facet,num_facet,analyze_grad,pooler_dropout, pooler_scalar_dropout):
    ...
    if params["attn"] and not model.use_bert:
        pooler = Pooler(project=False, d_inp=params["d_hid_attn"], d_proj=params["d_hid_attn"], pool_type=params["pool_type"])
        ...
        pooler = Pooler(
            ...
            previous_state_path=load_target_train_checkpoint, 
            unnorm_facet=unnorm_facet,
            analyze_grad = analyze_grad,
            embedder_for_grad = model.sent_encoder._text_field_embedder,
            pooler_dropout = pooler_dropout, 
            pooler_scalar_dropout = pooler_scalar_dropout
        )
...
def build_multiple_choice_module(task, d_sent, use_bert, params, load_target_train_checkpoint,unnorm_facet,num_facet, analyze_grad, embedder_for_grad, pooler_dropout, pooler_scalar_dropout):
    """ Basic parts for MC task: reduce a vector representation for each model into a scalar. """
    pooler = Pooler(num_facet=num_facet,
        project=not use_bert, d_inp=d_sent, d_proj=params["d_proj"], pool_type=params["pool_type"], previous_state_path=load_target_train_checkpoint, unnorm_facet=unnorm_facet, analyze_grad=analyze_grad, embedder_for_grad = embedder_for_grad, pooler_dropout = pooler_dropout, pooler_scalar_dropout = pooler_scalar_dropout)
...
def build_qa_module(task, d_inp, use_bert, params, load_target_train_checkpoint, unnorm_facet, num_facet, analyze_grad, embedder_for_grad, pooler_dropout, pooler_scalar_dropout):
